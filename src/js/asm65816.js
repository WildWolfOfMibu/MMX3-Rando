...
export class M65816 {
    ...
    parseNumber(num) {
        if (num.length === 0)
            throw new Error('No number to parse');
        let radix = 10;
        let numPart = num;
        if (num[0] === '$') {
            radix = 16;
        ...
        if (!isNaN(ret))
            return ret;
        if (this.symbols[num] !== undefined)
            return this.symbols[num];
        if (this.labels[num] === undefined)
            throw new Error(`Could not parse number ${num}`);
        let [blockName, offs] = this.labels[num];
        if (this.asm[blockName] === undefined)
            throw new Error(`Could not find block with name ${blockName}`);
        let romOffs = this.asm[blockName].placement + offs;
        return this.toBankAddr(romOffs);
    }
    getAbs(mnem, tokens, getSize) {
        if (tokens.length !== 1)
            throw new Error(`Incorrect args passed to ${mnem} abs`);
    ...
    getImm(mnem, tokens, getSize) {
        if (tokens.length !== 3 || tokens[1] !== '.')
            throw new Error(`Incorrect args passed to ${mnem} imm`);
        let size = 'bw'.indexOf(tokens[2]);
        if (size === -1)
            throw new Error(`Incorrect size for ${mnem} imm`);
        let value = getSize ? 0 : this.parseNumber(tokens[0]);
        let lb = value & 0xff;
        if (size === 0)
            return [lb];
    ...
    getRel(mnem, tokens, getSize) {
        if (tokens.length !== 1)
            throw new Error(`Incorrect args passed to ${mnem} rel`);
        if (getSize)
            return [0];
        let dest = tokens[0];
        ...
        if (Math.abs(rel) >= 0x80)
            throw new Error(`Relative branch for destination, ${dest}, too large: ${hexc(rel)}`);
        return rel >= 0 ? [rel] : [rel + 0x100];
    }
    adc(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to adc');
        if (tokens[0] === '#') {
    ...
    and(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to and');
        if (tokens[0] === '#') {
    ...
    asl(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to asl');
        if (tokens[0] === 'a') {
            return [0x0a];
        }
        else {
    ...
    bcc(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to bcc');
        return [0x90, ...this.getRel('bcc', tokens, getSize)];
    }
    bcs(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to bcs');
        return [0xb0, ...this.getRel('bcs', tokens, getSize)];
    }
    beq(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to beq');
        return [0xf0, ...this.getRel('beq', tokens, getSize)];
    }
    bit(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to bit');
        if (tokens[0] === '#') {
    ...
    bne(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to bne');
        return [0xd0, ...this.getRel('bne', tokens, getSize)];
    }
    bpl(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to bpl');
        return [0x10, ...this.getRel('bpl', tokens, getSize)];
    }
    bra(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to bra');
        return [0x80, ...this.getRel('bra', tokens, getSize)];
    }
    bvc(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to bvc');
        return [0x50, ...this.getRel('bvc', tokens, getSize)];
    }
    bvs(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to bvs');
    ...
    cmp(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to cmp');
        if (tokens[0] === '#') {
    ...
    inc(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to inc');
        if (tokens[0] === 'a') {
            return [0x1a];
        }
        else if (tokens[tokens.length - 1] == 'b') {
    ...
    jmp(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to jmp');
    ...
    jsr(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to jsr');
        if (tokens[tokens.length - 1] == 'w') {
    ...
    lda(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to lda');
        if (tokens[0] === '#') {
    ...
    ldx(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to ldx');
        if (tokens[0] === '#') {
    ...
    ldy(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to ldy');
        if (tokens[0] === '#') {
    ...
    ora(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to ora');
        if (tokens[0] === '#') {
    ...
    pea(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to pea');
        if (tokens[tokens.length - 1] == 'w') {
    ...
    rep(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to rep');
        if (tokens[0] === '#') {
    ...
    sbc(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to sbc');
        if (tokens[0] === '#') {
    ...
    sep(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to sep');
        if (tokens[0] === '#') {
    ...
    sta(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to sta');
    ...
    stx(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to stx');
        if (tokens[tokens.length - 1] == 'b') {
    ...
    sty(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to sty');
        if (tokens[tokens.length - 1] == 'w') {
    ...
    stz(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to stz');
        if (tokens[tokens.length - 1] == 'b') {
    ...
    tsb(tokens, getSize) {
        if (tokens.length === 0)
            throw new Error('No args passed to tsb');
        if (tokens[tokens.length - 1] == 'w') {
    ...
    addAsm(placeBank, placeAddr, asm, name) {
        ...
        if (!name)
            name = `asmBlock${this.randomNameIdx++}`;
