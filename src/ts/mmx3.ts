...
export type RandoOptions = {
    fullVersion: string;
    enemies_1hp: boolean;
    enemy_multipliers: string;
    fragile_walls: boolean;
    max_boss_hp: number;
    min_boss_hp: number;
    new_game_mode: string;
    no_knockback: boolean;
    random_boss_weakness: boolean;
    random_boss_drop: boolean;
    random_boss_hp: boolean;
    random_player_hp: boolean;
    random_enemies: boolean;
    romType: string;
    skip_intro: boolean;
    stage_sel_show_items: boolean;
    subweps_required: number;
    upgrades_required: number;
    zero_damage: boolean;
}

export function randomize(rom: number[], rng: () => number, opts: RandoOptions) {
    /*
    Initial rom changes
    */
    ...
    if (opts.random_boss_drop) {
        ...
        let subwepToStage: { [key: number]: number } = {
            0: 3,
            1: 0,
            2: 4,
            3: 5,
